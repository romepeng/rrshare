...
import pandas as pd
from rrshare.rqUtil import demjson
import requests
from bs4 import BeautifulSoup

from rrshare.swlIndex.cons import sw_headers, sw_payload, sw_url
...
def sw_index_third_info(level="level3") -> pd.DataFrame:
    """
    乐咕乐股-申万三级-分类
...
if __name__ == "__main__":
    ...
    sw_index_daily_df = sw_index_daily(
        symbol="801010", start_date="2021-05-01", end_date="2022-05-06"
    )
    print(sw_index_daily_df)
    print(sw_index_daily_df.columns)
    sw_index_daily_indicator_df = sw_index_daily_indicator(symbol="801010",start_date="2021-05-01",end_date="2022-05-31")    
    print(sw_index_daily_indicator_df)

    sw_index_third_info_df = sw_index_third_info(level='level2')
